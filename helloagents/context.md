...
姿态模型仍然是 `StaticMlpPolicy` 静态 MLP 权重，当前结构为 72-128-128-3，对应 8 帧历史输入。最近一次优化联合调整了历史窗口、NN 历史策略参数、速度外环、模型力矩缩放和 PWM 混控参数；独立仿真仓库的 `flight_control_eval` 当前 5/5 场景稳定，平均分 `90.499/100`。
